---
# Display name
title: Dr. Chiara Herzog

# Is this the primary user of the site?
superuser: true

# Role/position/tagline
role: Principal Investigator

# Organizations/Affiliations to show in About widget
organizations:
  - name: King's College London
    url: https://www.kcl.ac.uk/research/herzog-group

# Short bio (displayed in user profile at end of posts)
bio: Leading research in epigenetics to transform how we understand and modulate ageing.

# Interests to show in About widget
interests:
  - Epigenetics
  - Biomarkers
  - Ageing research
  - Immunity

# Education to show in About widget
education:
  courses:
    - course: PhD in Neuroscience
      institution: The University of Edinburgh
      year: 2019
    - course: MSc by Research in Integrative Neuroscience
      institution: The University of Edinburgh
      year: 2015
    - course: BSc in Molecular Medicine
      institution: Innsbruck Medical University
      year: 2014

# Social/Academic Networking
social:
  - icon: hero/envelope
    link: 'mailto:chiara.herzog@kcl.ac.uk'
  - icon: brands/google-scholar
    link: https://scholar.google.com/citations?user=rZe4io4AAAAJ
  - icon: brands/github
    link: https://github.com/chiaraherzog
  - icon: brands/linkedin
    link: https://www.linkedin.com/in/chiara-herzog
  - icon: brands/orcid
    link: https://orcid.org/0000-0002-1572-498X
  # Link to a PDF of your resume/CV from the About widget.
  # To use: copy your resume to `static/uploads/resume.pdf`, enable `ai` icons in `params.yaml`,
  # and uncomment the lines below.
  - icon: brands/cv
    link: uploads/CV_CHerzog.pdf

# Highlight the author in author lists? (true/false)
highlight_name: true

# Author's website URL
website: ""

# User groups
user_groups:
  - Principal Investigator

author_content:
  publications:
    enable: true
    keys: "BuccalBC,TGIF,TGAtlas,functageingsigs"
  news:
    enable: true
    latest: 1
    view: "article-grid"
    css_class: "grid-3"
  events:
    enable: true
    latest: 3
    view: "article-grid"
    css_class: "grid-3"

---

Dr. Chiara Herzog leads the Epigenetics and Ageing lab at King's College London, within the Department of Twin Research and Genetic Epidemiology. With over 10 years of experience in interdisciplinary research, she has pioneered novel insights into how our epigenome records past exposures, present cellular state, and future disease risk. 

## Awards & Recognition

- 2025 - Wellcome Trust Career Development Award
- 2025 - Rising Star, Longevity Summit Dublin
- 2025 - Prof. Anthony Mellows Medal
- 2024 - King's Prize Fellowship
- 2024 - Austrian Association of Molecular Life Sciences and Biotechnology (ÖGMBT) Award in Basic Science
- 2024 - Austrian Gynaecological Oncology Working Group Young Scientist Award
- 2020 - Falling Walls Winner - Austria
